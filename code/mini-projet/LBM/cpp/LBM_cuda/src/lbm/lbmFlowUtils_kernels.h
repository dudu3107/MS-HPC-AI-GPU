                                   ...
                                   real_t *feq_d)
{
  ...
  if( (i<nx) && (j<ny) ){//&& (k<npop)){
    int index = i + nx * j;
      ...
      //real_t cu = 3 * (v(k,0) * ux_d[index] + 
      //                  v(k,1) * uy_d[index]);
      
      //int index_f = index + k * nx * ny;
      //feq_d[index_f] = rho_d[index] * t(k) * (1 + cu + 0.5*cu*cu - usqr);
                                       ...
                                       real_t *fout_d)
{
  ...
  // TODO   This can possibly be optimized by creating threads inside the inner loop
  const int nx = params.nx;
  const int ny = params.ny;
  const int npop = LBMParams::npop;
  const int nxny = nx*ny;
